...
## Introduction
  ...
  Throughout the NFL's history, it has always been plagued by injuries. However, the NFL is always striving to improve their player's safety and as a result has made vast improvements when it comes to player healthcare, recovery, and medical procedures. Despite these efforts, players continue to be injured and, in recent seasons, injuries appear to be increasing. 
  
  In this analysis, I will be answering whether or not player injuries decrease throughout a season and if there is anything we can do to lessen the amount of injuries overall. 
...
* ARE injuries actually getting better?

* Do player injuries increase or decrease throughout a season?

* When during the season do most players get injured?
...
Here are the different types of injuries in the dataset. There are 105 total injuries with all of them happening from the lower limbs. 
...
```
(fact plot graph that shows which positions are being injured the most)

```{r, echo = FALSE, message=FALSE, warning=FALSE}
ggplot(play_list, aes(x = Position, fill = Position)) +
  geom_bar(alpha = (1/3), position = "stack") +
...
Here are all of the different stadiums where the injuries happened. (this is where kableextra goes)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(play_list, aes(x = Position, y = FieldType)) +
  geom_area() +
...
```{r,echo = FALSE, message=FALSE, warning=FALSE}
ggplot(play_list) +
  geom_col(aes(x = PlayerGame, y = Position), fill = "purple1") 
