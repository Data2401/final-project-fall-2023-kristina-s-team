...
Here we can see that many of the different stadium types are primarily either indoors or outdoors.

Something of note is that there is a strange category called 'Cloudy' which is more than likely a mistake when inputting the data. There are also missing entries for the stadium type and the 'Heinz Field' category. This stadium is an outdoor stadium but it is weird that they specifically named this one stadium.  
...
Next, I created a horizontal bar graph to analyze the frequency of injuries across different positions. The y-axis represents 'Position,' while the x-axis corresponds to the total number of injuries. 

Upon examining the graph, it is evident that wide receivers experience the highest frequency of injuries while defensive backs experience the lowest frequency of injuries. What is interesting to note is that wide receivers, outside linebackers, and cornerbacks are the top 3 positions that tend to be injured the most. Wide receivers and cornerbacks are usually paired against each other with wide receivers trying to gain yards and cornerbacks trying to stop them. This means that there are plenty of opportunities for injuries between the two positions to happen. Outside Linebackers tend to be lined up against either running backs or tight ends 
...
Now we're getting somewhere. This helps to prove that of the 105 injuries in the dataset over half of them happen on synthetic turf. However, there
...
Now this is very interesting. Looking at the injuries per position based on each field type we can see that position injuries heavily vary depending on which surface they played on. There were four positions that had the majority of their injuries on synthetic turf: Defensive Backs, Defensive Tackles, Inside Linebackers, and Cornerbacks. While there were three positions that had the majority of their injuries on natural grass: Middle Linebackers, Tackles, and Outside Linebackers. The rest of the players injuries were very close with three of the positions injuries being tied: Centers, Tight Ends, and Linebackers. 
...
Yes, there is a way to lessen the amount of injuries in the NFL and that is for all NFL stadiums to use natural grass. While there isn't a way to completely get rid of injuries, the next best thing we can do is to try to lessen the amount of injuries for the player's safety. The
